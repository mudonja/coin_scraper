        ...
        if(soup.find('h2', {'class': "f4 text-normal flex-auto pr-2"})):
            ...
            if (pinned_repos == "pinned repositories"):
                ...
                for link in links:
                    ...
                    cmd = "git clone --progress", github_url + str(link.get('href')) + '.git ', currency_dir, "2>>~/currency-clone.log"
                    os.system(cmd)
            ...
            print("Repository")
            cmd = "git clone --progress", str(val)[:8]+usrname+":"+passwrd+"@"+str(val)[8:]+'.git ', currency_dir, "2>>~/currency-clone.log"
            os.system(cmd)
            Download_counter = Download_counter + 1
